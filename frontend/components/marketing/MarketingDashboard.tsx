...
export function MarketingDashboard({ accountId, onChatMessage }: Props) {
  ...
  const { state, refresh, fetchActions, fetchAnalysis } = useMarketingData(accountId)
  ...
  const handleConnectYoutube = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/marketing/youtube/oauth/start?account_id=${aid}`)
    ...
    return (
      ...
      >
        <div className="flex items-center gap-5 text-sm">
          ...
          {netSubs !== null && (
            <span className={`text-xs ${netSubs >= 0 ? "text-green-600" : "text-red-500"}`}>
              구독자 {netSubs >= 0 ? "+" : ""}{fmt(netSubs)}
            </span>
          )}
    ...
    <div className="mb-6 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      ...
      {!state.loading && state.error && (
        <div className="p-8 text-center">
          ...
          <button onClick={refresh} className="mt-2 text-xs text-rose-500 underline">
            다시 시도
          </button>
        </div>
      )}
