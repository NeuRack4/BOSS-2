...
export const IntegrationsModal = ({ open, onClose }: Props) => {
  ...
  const fetchAll = async () => {
    const [naver, ig, yt] = await Promise.all([
      fetch(`/api/integrations/naver_blog?account_id=${accountId}`).then((r) =>
        r.json(),
      ),
      fetch(`/api/integrations/instagram?account_id=${accountId}`).then((r) =>
        r.json(),
      ),
      fetch(`/api/integrations/youtube?account_id=${accountId}`).then((r) =>
        r.json(),
      ),
    ]);
  ...
  const saveNaver = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/integrations/naver_blog", {
        method: "PUT",
        body: form,
      });
      if (!res.ok) {
  ...
  const disconnectNaver = async () => {
    ...
    await fetch(`/api/integrations/naver_blog?account_id=${accountId}`, {
      method: "DELETE",
    });
    setBlogId("");
    setCookieFile(null);
  ...
  const disconnectInstagram = async () => {
    ...
    await fetch(`/api/integrations/instagram?account_id=${accountId}`, {
      method: "DELETE",
    });
    setMetaToken("");
    setMetaIgToken("");
  ...
  const connectYouTube = async () => {
    const res = await fetch(
      `/api/marketing/youtube/oauth/start?account_id=${accountId}`,
    );
  ...
  const disconnectYouTube = async () => {
    ...
    await fetch(`/api/integrations/youtube?account_id=${accountId}`, {
      method: "DELETE",
    });
    fetchAll();
  };
        ...
        <div className="flex flex-col gap-4 overflow-y-auto flex-1 min-h-0 pr-0.5">
          ...
          <GuideBox tab="instagram" title="토큰 발급 방법 (최초 1회 설정)">
            ...
            <Step n={2} tab="instagram">
              ...
              <Hint>
                인스타그램 앱 → 프로필 → 설정 → 계정 → "연결된 계정" → Facebook
                선택 후 페이지 연결
              </Hint>
            </Step>
            ...
            <Step n={5} tab="instagram">
              ...
              <strong>"Graph API Explorer"</strong> 클릭 → 우측 앱 드롭다운에서
              내 앱 선택 → <strong>"권한 추가"</strong>를 눌러 아래 3가지를 모두
              체크 후 <strong>"토큰 생성"</strong> 클릭 → Facebook 로그인 후
              권한 허용
            ...
            <Step n={6} tab="instagram">
              ...
              확인
              <Hint>
                developers.facebook.com → 해당 앱 → 설정 → 기본 설정 → "앱 ID"와
                "앱 시크릿 코드" 값 복사
              </Hint>
            </Step>
      ...
      {activeTab === "naver" && (
        <div className="flex flex-col gap-4 overflow-y-auto flex-1 min-h-0 pr-0.5">
          ...
          <GuideBox tab="naver" title="쿠키 발급 및 연결 방법">
            ...
            <Step n={6} tab="naver">
              <strong>블로그 ID 확인</strong> — 네이버 블로그에 접속해 내 블로그
              주소의 마지막 부분이 ID입니다.
              <Hint>
