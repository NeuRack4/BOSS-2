...
type OverviewData = {
  year: number;
  month: number;
  sales: { total: number; prev_total: number; change_rate: number | null; daily_avg: number };
...
function ChangePill({ rate }: { rate: number | null }) {
  if (rate === null) return <span className="text-[11px] text-[#aaa]">전월 데이터 없음</span>;
  if (rate === 0) return (
  ...
  const up = rate > 0;
  return (
    <span className={`inline-flex items-center gap-0.5 rounded-[3px] px-1.5 py-0.5 text-[11px] font-mono font-semibold ${up ? "bg-[#e8edd8] text-[#4a5c28]" : "bg-[#f4e8e0] text-[#8a3a28]"}`}>
...
function DailyBarChart({ series }: { series: DayPoint[] }) {
  ...
  const H = 110, PAD_T = 8, PAD_B = 24, PAD_X = 4;
      ...
      <svg width={W} height={H} style={{ display: "block" }}>
        ...
        {series.map((d, i) => {
          ...
          return (
            <g key={i}>
              <rect
                x={x + 0.75} y={y} width={barW} height={barH}
                rx={2}
              ...
              {/* 오늘 표시 */}
              {isToday && (
                <text x={x + barW / 2 + 0.75} y={H - 6} textAnchor="middle" fontSize={9} fill="#7f8f54" fontWeight="600">
                  오늘
                </text>
              )}
            </g>
        ...
        {series
          ...
          .map((d) => {
            ...
            return (
              <text key={d.day} x={x} y={H - 6} textAnchor="middle" fontSize={9} fill="#aaa" fontFamily="monospace">
                {d.day}일
              </text>
            );
          })}
        ...
        {/* 데이터 없을 때 안내 */}
        {!hasData && (
          <text x={W / 2} y={PAD_T + plotH / 2} textAnchor="middle" fontSize={12} fill="#c8c0b0">
            이번 달 매출 데이터 없음
          </text>
        )}
      </svg>
...
export function RevenueStatsPanel({ accountId }: { accountId: string }) {
  ...
  if (noData) return (
    <div className="flex flex-col items-center gap-4 py-10 text-center">
      ...
      <div>
        <p className="text-[14px] font-semibold text-[#2e2719]">아직 매출 데이터가 없어요</p>
        <p className="mt-1 text-[12px] text-[#8c7e66]">
          챗봇에서 오늘 매출을 입력하면<br />여기에 통계가 자동으로 쌓여요.
        </p>
      </div>
      <div className="rounded-[5px] border border-[#d0cbbf] bg-[#faf8f3] px-4 py-3 text-left text-[12px] text-[#5a5040]">
        ...
        <p className="text-[#6a7843]">"오늘 아메리카노 30잔, 라떼 20잔 팔았어"</p>
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {fewData && (
        <div className="flex items-center gap-2 rounded-[5px] border border-[#d0cbbf] bg-[#faf8f3] px-3 py-2">
          ...
          <p className="text-[12px] text-[#5a5040]">
            데이터가 쌓일수록 분석이 정확해져요.{" "}
            <span className="font-semibold text-[#4a5c28]">매출을 더 입력해보세요!</span>
          </p>
        </div>
      )}
      ...
      <div className="flex items-center gap-2">
        <span className="text-[13px] font-semibold text-[#2e2719]">{monthLabel} 현황</span>
        <span className="font-mono text-[11px] text-[#aaa]">일평균 {fmt(sales.daily_avg)}원</span>
      </div>

      {/* 요약 3카드 */}
      <div className="grid grid-cols-3 gap-2">
        {[
          { label: "매출",   value: sales.total,  rate: sales.change_rate,  color: "#4a5c28", bg: "#f0f4e8" },
          { label: "비용",   value: costs.total,  rate: costs.change_rate,  color: "#8a3a28", bg: "#f9f0ec" },
          { label: "순이익", value: profit.total, rate: profit.change_rate, color: profit.total >= 0 ? "#4a5c28" : "#8a3a28", bg: profit.total >= 0 ? "#f0f4e8" : "#f9f0ec" },
        ].map((c) => (
          <div key={c.label} className="rounded-[5px] border border-[#d0cbbf] bg-white p-3">
            <p className="text-[11px] text-[#8c7e66]">{c.label}</p>
            <p className="mt-1 text-[15px] font-bold" style={{ color: c.color }}>{fmt(c.value)}원</p>
