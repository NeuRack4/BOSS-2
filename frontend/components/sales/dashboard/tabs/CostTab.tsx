...
// ── 전월 비교 바 ───────────────────────────────────────────────────────────────
function CompareBar({
  label, current, prev, higherIsBetter,
}: {
  label: string
  current: number
  prev: number
  ...
  const insight =
    changeRate === 0 ? `지난달과 동일한 ${label}이에요` :
    isGood
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold text-slate-600">{label} 전월 비교</span>
      ...
      <div className="flex items-center gap-2">
        <span className="w-12 shrink-0 text-right text-[10px] text-slate-500">이번달</span>
        ...
        <span className="w-16 shrink-0 text-[10px] font-medium text-slate-700">{fmt(current)}원</span>
      </div>

      {/* 지난달 */}
      <div className="flex items-center gap-2">
        <span className="w-12 shrink-0 text-right text-[10px] text-slate-400">지난달</span>
        <div className="flex-1 h-2.5 overflow-hidden rounded-full bg-slate-100">
          <div
        ...
        <span className="w-16 shrink-0 text-[10px] text-slate-400">{fmt(prev)}원</span>
      </div>
...
function ProfitRateBar({ salesTotal, costsTotal }: { salesTotal: number; costsTotal: number }) {
  ...
  const rateLabel =
    profitRate >= 40 ? "우수한 수익률이에요 🎯" :
    profitRate >= 25 ? "양호한 수익률이에요" :
    profitRate >= 10 ? "수익률 개선 여지가 있어요" :
    profitRate >= 0  ? "수익률이 낮아요 — 비용 점검이 필요해요" :
    "이번달 적자 상태예요 ⚠️"

  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm space-y-2">
      <div className="flex justify-between">
        <span className="text-xs font-semibold text-slate-600">이번달 수익률</span>
        <span className={`text-lg font-extrabold ${profitRate >= 0 ? "text-green-600" : "text-red-500"}`}>
          {profitRate}%
        </span>
      </div>
  ...
  if (!overview) {
    return (
      <div className="p-8 text-center">
        ...
        <p className="mt-2 text-sm font-medium text-slate-600">비용 데이터가 없어요</p>
        <p className="mt-1 text-xs text-slate-400">챗봇으로 원자재·인건비 등을 기록하면 수익률 분석이 가능해요</p>
      </div>
    )
  }
