...
function MenuRow({
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg px-3 py-2 hover:bg-slate-50">
      ...
      {/* 마진율 % */}
      <div
        className={`w-10 shrink-0 text-right text-xs font-bold ${color.text}`}
      >
        {rate.toFixed(0)}%
      </div>
      ...
      {view === "margin" && (
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="flex items-center gap-3 border-b border-slate-100 px-3 py-2">
            ...
            <div className="flex-1 text-[10px] font-semibold text-slate-400">
              마진율
            </div>
            <div className="w-10 text-right text-[10px] font-semibold text-slate-400">
              %
            </div>
          ...
          {menusWithMargin.map((menu) => (
            ...
            >
              ...
              <p className="shrink-0 text-xs">
                ...
                <span
                  className={`font-bold ${MARGIN_COLOR(menu.margin_rate ?? 0).text}`}
                >
