      ...
      {view === "quadrant" && (
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm space-y-2">
          ...
          {menusWithMargin.map(menu => (
            <div key={menu.id} className="flex items-center justify-between gap-2">
              <span className="truncate text-xs text-slate-700">{menu.name}</span>
              <div className="flex items-center gap-2 shrink-0">
                <span className="text-[10px] text-slate-400">{fmt(menu.price)}원 / {menu.margin_rate?.toFixed(0)}%</span>
