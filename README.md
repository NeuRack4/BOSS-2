...
- **Recruitment 3종 플랫폼 공고 단일 카드 + HTML 포스터 v2.3** — 당근알바/알바천국/사람인 세트를 `[JOB_POSTINGS]` 마커로 동시 생성 → `job_posting_set` 단일 artifact (탭 토글 UI). 모든 필수 정보(매장명·급여·근무요일·시각·모집기간 등) 확정 시만 생성, 미확정 항목은 CHOICES 로 되물음. HTML 포스터는 GPT-4o standalone HTML 로 플랫폼별 비율(1:1 / 4:5 / 3:2) 생성 후 Supabase Storage + `artifacts.content` 이중 저장. 상세 모달에서 `iframe srcDoc` 렌더링 + JPG Download(2× 해상도). 인건비는 `_recruit_calc.py` 에서 연도별 최저임금 + 주휴수당 + 4대보험 의무 시뮬레이션.
...
현재 버전: **2.3.0** — 자세한 변경 내역은 [CHANGELOG.md](./CHANGELOG.md) 참고.
