...
- **Bento 대시보드 v1.0.0** — `/dashboard` 가 React Flow 캔버스에서 **12-컬럼 Bento Grid** 로 전환. 좌측 `ProfileMemorySidebar` (Profile / Long-term Memory / Memos 3:3:3) + 중앙에 `ChatCenterCard` (풀기능 **InlineChat** — 구 ChatOverlay 1,641 줄을 카드 안으로 이식, 파일 업로드·OCR·choices·inline sales/cost 테이블·Markdown·리뷰 카드 전부 동작) + 4개 `DomainCard` (Active/Due/Recent 3-열 KPI + 최근 5 pill) + 하단에 `PreviousChatCard` / `ScheduleCard` / `ActivityCard`. 매 세션 로드 시 도메인별 10문항 풀에서 랜덤 샘플링된 제안 프롬프트 4개 표시. 모든 카드 `rounded-[5px]` + 빈 상태 문구 `Nothing here yet` 통일 + UI 전역 영어화
- **Domain Kanban v1.0.0** — DomainCard 클릭 시 `/recruitment` / `/marketing` / `/sales` / `/documents` 로 진입하면 해당 도메인 서브허브를 컬럼으로 펼친 Kanban 보드. 드래그 시 `PATCH /api/kanban/move` 로 `artifact_edges.relation='contains'` 부모 교체. CSS 변수 기반 theme tokens 로 light/dark 둘 다 가독
...
- **전역 검색 팔레트** (`⌘K` / `Ctrl+K`): 제목·본문·메모·metadata 하이브리드(벡터+FTS) 검색, 결과 클릭 시 `boss:focus-node` 이벤트로 해당 노드로 이동 (영어 UI)
- **대시보드 모달 6종 (720×560 통일)**: `ChatHistoryModal` (세션 리스트 + 삭제 버튼) / `ScheduleManagerModal` (리스트·캘린더 토글) / `ActivityModal` / `ProfileModal` / `LongTermMemoryModal` / `MemosModal` — `createPortal(..., document.body)` 렌더 (헤더의 `backdrop-filter` containing block 이슈 해결), `variant="dashboard"` 고정 팔레트
- **새 세션 / 기존 세션 로드**: ChatCenterCard 우상단 `New Session` 버튼 + ChatHistoryModal 에서 세션 선택 시 `requestLoadSession(id)` → InlineChat 이 즉시 하이드레이트
...
│   │   ├── dashboard/           # Bento Grid 대시보드 (v1.0.0~)
│   │   └── [domain]/            # /recruitment · /marketing · /sales · /documents (Kanban)
│   ├── components/
│   │   ├── bento/               # BentoGrid / ChatCenterCard / DomainCard / ScheduleCard /
│   │   │                        # ActivityCard / PreviousChatCard / ProfileMemorySidebar /
│   │   │                        # KanbanBoard · KanbanColumn · KanbanCard / DomainPage
│   │   ├── chat/                # InlineChat (풀기능 인라인 채팅) + ChatContext + 카드 렌더러
│   │   ├── layout/              # Header + 대시보드 모달 6종
│   │   │                        # (Schedule / Activity / ChatHistory / Profile / LongTermMemory / Memos)
│   │   ├── search/              # SearchPalette (⌘K)
│   │   ├── canvas/              # (legacy) React Flow 컴포넌트 — /canvas-legacy 삭제 후 소스만 보존
│   │   └── ui/                  # Modal (portal + variant=sand|dashboard) · shadcn 공통
│   └── lib/
│       ├── supabase.ts
│       └── api.ts
│
...
현재 버전: **1.0.0** — 자세한 변경 내역은 [CHANGELOG.md](./CHANGELOG.md) 참고.
