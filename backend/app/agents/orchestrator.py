    ...
    resp = await chat_completion(
        messages=[
            {"role": "system", "content": (
                ...
                "- marketing: SNS·광고·이벤트 기획·블로그·리뷰 답글 작성 + **광고 이미지·썸네일·배너 생성**\n"
...
[규칙]
1. 도구의 required 파라미터가 메시지+히스토리로 채워지면 즉시 호출.
2. required 파라미터가 부족하면 **도구를 호출하지 말고** 본문으로 한 번만 되물어보세요 (짧게, 한 질문만).
3. 객관식으로 되물을 때는 보기를 `[CHOICES]\\n보기1\\n보기2\\n...\\n[/CHOICES]` 형식으로 감싸세요.
4. 사용자가 사소한 인사/잡담/도구 외 질문이면 도구 호출 없이 자연어로 짧게 답하세요.
5. 날짜 표현(내일·다음주 금요일 등)은 시스템에 주어진 오늘 기준으로 YYYY-MM-DD 로 환산해 파라미터에 넣으세요.
6. 여러 도구가 동시에 필요하면 parallel tool call 로 함께 호출해도 됩니다 (예: 공고 + 면접 질문).
"""


async def _dispatch_via_tools(
    account_id: str,
    message: str,
    history: list[dict],
    rag_context: str,
    ...
    """V2 도메인 경로 — OpenAI function-calling 으로 capability 선택·실행.

    반환:
      - str: tool_calls 실행 결과 (단일/다중 합성) 또는 LLM 의 자연어 응답 (되묻기 등)
      - None: tools 가 하나도 등록되지 않음 → legacy 경로 사용
    """
    import json
    ...
    try:
        ...
        log.exception("[tools] dispatch failed: %s", exc)
        return None

    msg = resp.choices[0].message
    tool_calls = getattr(msg, "tool_calls", None) or []
    if not tool_calls:
        # 도구 없이 자연어 답변 (되묻기 / 짧은 응답)
        log.info("[tools] account=%s no_tool_call", account_id)
        return (msg.content or "").strip()

    log.info(
        "[tools] account=%s tool_calls=%s",
        account_id,
        [tc.function.name for tc in tool_calls],
    )

    # 각 tool_call 실행 (순차 — 현재 capability 들은 상호 독립)
    results: list[tuple[str, str]] = []  # (capability_name, reply)
    for tc in tool_calls:
        name = tc.function.name
        entry = dispatch.get(name)
        if not entry:
            results.append((name, f"_(알 수 없는 도구: {name})_"))
            continue
        try:
        ...
        try:
            ...
            log.exception("[tools] arg mismatch for %s: %s", name, exc)
            results.append((name, f"_(도구 '{name}' 인자 오류: {str(exc)[:160]})_"))
            continue
        except Exception as exc:
            log.exception("[tools] handler %s crashed", name)
            results.append((name, f"_(도구 '{name}' 실행 중 오류: {str(exc)[:160]})_"))
            continue
        results.append((name, reply))

    if len(results) == 1:
        return _extract_and_save_nickname(account_id, results[0][1])

    # 다중 tool 결과 합성 — CHOICES 가 남아있으면 섹션 passthrough
    if any(_CHOICES_RE.search(r) for _, r in results):
        parts = [f"### {name}\n{r}" for name, r in results]
        return "\n\n".join(parts)
    # 정상 합성
    per = {name: r for name, r in results}
    return await _synthesize_cross_domain(message, per, nick_ctx, account_id)
