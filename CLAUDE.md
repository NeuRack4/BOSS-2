...
**Documents 서브허브 역할** (v1.3 재정의):

| 서브허브       | 역할                                                                                                                          | 담당 타입                                                          |
| -------------- | ----------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| **Review**     | 공정 중립이 필요한 서류의 **작성·검토**. 계약서·제안서 초안 생성 + 업로드된 기존 서류의 공정성 분석 (갑/을 비율 + 위험 조항). | `contract` · `proposal` · `analysis`                               |
| **Tax&HR**     | 인사평가 관리 + 세무 관련 문서 (채용 제외).                                                                                   | `checklist` · `guide` (추후 인사평가서·급여·세무 캘린더 확장 예정) |
| **Legal**      | 법률 자문. 법령 RAG + `legal_annual_values` (연도별 최저임금·세율) 기반 조언 + 면책 고지.                                     | `legal_advice`                                                     |
| **Operations** | 서류 초안 작성·행정 업무. 견적서·공지문 + 국가 지원사업 신청서·행정 처리 신청서.                                              | `estimate` · `notice` (추후 지원사업·행정 신청서 확장 예정)        |
